...
3. **`moves.increment`** is a pure function from state and args to an *outcome*. Outcomes are built with the `move` helper: `move.endTurn(...)`, `move.win(...)`, `move.invalid(...)`, etc. You never mutate `G` directly — you return a patch describing what changed. See [moves and outcomes](/concepts/gamekit-moves-and-outcomes) for the full set.
...
moves: {
  increment: move({
    ...
    run({ G, move, player }) {
      const value = G.value + 1;
      if (value >= 3) return move.win(player.id, { value });
      return move.endTurn({ value });
    },
  }),
  resetBoard: move({
