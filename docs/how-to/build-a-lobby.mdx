...
- [How-to: configure match settings](/how-to/configure-match-settings) — host-mutable settings form (turn timer, variant, etc.) that locks into `match.config` at start.
- [How-to: enforce turn timeouts](/how-to/enforce-turn-timeouts) — server-authoritative deadline enforcement: declare a `phase.deadline`, write an `onTimeout` handler, the framework fires the trigger when wall-clock elapses.
