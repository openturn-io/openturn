...
const profile = defineProfile<{ wins: number }>({
  ...
  commit: ({ profile, result }) => {
    // Gamekit sets `result.winner` when a move calls `move.win(playerID)`.
...
export const game = defineGame({
  ...
  moves: ({ move }) => ({
    tap: move({
      run: ({ player, move: m }) => m.win(player.id, { over: true }),
    }),
  }),
  phases: {
    play: {
...
That's the full wiring. When `move.win(...)` fires, gamekit sets `result.winner` and the engine calls `profile.commit`. The host persists the returned delta.
