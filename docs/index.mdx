...
Want to see a fully realized openturn game? The [**Splendor**](/examples#featured-splendor) example is the flagship — 2–4 player hosted multiplayer with hidden per-player state, three difficulty-tiered AI bots in the lobby, and a polished tabletop UI. Run it with `bun --filter @openturn/example-splendor-app dev`.
