...
| Helper | Compiles to |
| --- | --- |
| `move.stay(patch?)` | same phase, `turn: "preserve"` |
| `move.endTurn(patch?)` | same phase, `turn: "increment"` |
| `move.goto(phase, patch?, { endTurn? })` | target phase |
| `move.win(winnerID, patch?)` | terminal `__gamekit_won` with `result.winner` |
| `move.draw(patch?)` | terminal `__gamekit_drawn` with `result.draw` |
| `move.invalid(reason?, details?)` | `rejectTransition(reason, details)` |

Every non-invalid outcome accepts `{ enqueue: [...] }` for internal follow-up events.
...
export const game = defineGame({
  ...
  setup: () => ({ over: false }),
  moves: ({ move }) => ({
    tap: move({ run: ({ player, move: m }) => m.win(player.id) }),
  }),
});
```

Gamekit's result is `{ draw?: true; winner?: PlayerID }`, so `commit` reads `result.winner`. Also re-exported: `applyProfileDelta`, `computeProfileCommit`, `validateProfileDelta`, and the delta grammar types. See [concepts: persistent profiles](/concepts/persistent-profiles) and [how-to: persist player state](/how-to/persist-player-state).
...
- **`GamekitInternalState`** — the private compartment gamekit owns inside a game's `G`. Exposed at runtime as `G.__gamekit`. Carries the optional `result: GamekitResultState | null` plus arbitrary JSON-shaped slots gamekit reserves for future internal state. You should not write to it directly; use `move.win` / `move.draw` / `move.invalid` instead.
- **`GamekitResultState`** — `{ draw?: true; winner?: PlayerID }`.
