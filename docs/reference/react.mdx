...
Use `room.bridge.capabilities.enable(preset, handler, options?)` — or the `useCapability(...)` hook below — to advertise preset shell utilities (`new-game`, `share-invite`, `current-turn`, `rules`).
...
### `useCapability(bridge, preset, handler, options?)`

Enable a single shell capability preset for the lifetime of the component. Handles the effect, disposer, and null-bridge case.

```tsx
useCapability(room.bridge, "new-game", () => match.reset());

// Pass `false` / null for `preset` to conditionally skip registration —
// useful when the capability depends on data that may be unavailable.
useCapability(
  room.bridge,
  inviteURL !== null && "share-invite",
  async () => { await navigator.clipboard?.writeText(inviteURL ?? ""); },
);

// `options` accepts `{ disabled?, badge? }` — the same `CapabilityEnableOptions`
// from `@openturn/bridge`.
useCapability(room.bridge, "current-turn", () => {}, { badge: turnNumber });
```

`preset` is one of the `BridgeCapabilityPreset` values — `"share-invite"`, `"current-turn"`, `"new-game"`, `"rules"`. The handler closure is always up-to-date via an internal ref, so no `useCallback` is needed. The effect re-registers only when the bridge, preset, or option fields (`badge`, `disabled`) change.
...
Opinionated lobby UI component. Pass it `room.lobby`. Hosted shells should expose invite sharing through the bridge capability menu instead of the embedded lobby UI.
