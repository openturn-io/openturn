...
- Instead of outcomes like `move.win(playerID)`, you return `{ G, result, turn }` from the matching transition branch.
...
run({ G, args, move, player }) {
  ...
  if (getWinner(board) !== null) return move.win(player.id, { board });
  if (isBoardFull(board)) return move.draw({ board });
