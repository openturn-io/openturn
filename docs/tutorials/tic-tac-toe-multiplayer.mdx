...
description: Reuse the tic-tac-toe game package from tutorial 1, host it on a local dev server, then deploy to Cloudflare.
sidebarTitle: Multiplayer
---

This tutorial assumes you finished [tic-tac-toe with gamekit](/tutorials/tic-tac-toe-gamekit) and have a working `@my/tic-tac-toe-game` package. Here we host the same game with `@openturn/server`, connect to it from a React app with `<OpenturnProvider cloud>` + `useRoom`, and deploy it to Cloudflare with `openturn deploy`.
...
- A Cloudflare Worker script with a Durable Object namespace for your rooms (`GameRoom`).
- An `openturn.manifest.json` with `gameKey`, `schemaVersion`, a SHA256 digest, and Worker bindings.

`openturn deploy` uploads the bundle to the cloud control plane, which provisions the Durable Objects and returns a play URL. That play URL embeds a backend fragment that `@openturn/bridge` decodes to find the WebSocket endpoint.

See [how-to: deploy to Cloudflare](/how-to/deploy-to-cloudflare) for flags and troubleshooting.
