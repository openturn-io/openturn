...
- Updated dependencies [56b512e]
  - @openturn/core@0.9.0
  - @openturn/deploy@0.9.0
  - @openturn/inspector-ui@0.9.0
  - @openturn/protocol@0.9.0
  - @openturn/react@0.9.0
  - @openturn/server@0.9.0
  - @openturn/bridge@0.9.0
  - @openturn/json@0.9.0
...
- Updated dependencies [da70cae]
  ...
  - @openturn/react@0.8.0
  - @openturn/deploy@0.8.0
  - @openturn/inspector-ui@0.8.0
  - @openturn/json@0.8.0
...
- 2dbd570: Fix `npx @openturn/cli` and `bun install -g @openturn/cli` failing to expose a working `openturn` binary.

  Two unrelated bugs landed together in 0.6.0:

  - **Registry metadata pointed at the wrong file.** The `prepack`/`postpack` script pair merged `publishConfig` (with `dist/` paths) into `package.json` for `npm pack`, then restored the dev-mode `src/` paths immediately after. But `npm publish` reads the registry-bound metadata from on-disk `package.json` _after_ `postpack` runs, so the registry received `bin: 'src/index.ts'` — a path that isn't shipped in the tarball. npm install silently skipped creating `node_modules/.bin/openturn`. Fix: drop the per-package `postpack`, leave the `prepack`-modified `package.json` in place through `npm publish`, and restore via `git restore packages/*/package.json` in the root `release` script after `changeset publish` finishes.
  ...
  - @openturn/core@0.6.1
  - @openturn/deploy@0.6.1
  - @openturn/inspector-ui@0.6.1
  - @openturn/json@0.6.1
  - @openturn/protocol@0.6.1
  - @openturn/react@0.6.1
...
- Updated dependencies [ffb51b3]
  ...
  - @openturn/deploy@0.6.0
  - @openturn/react@0.6.0
  - @openturn/core@0.6.0
  - @openturn/inspector-ui@0.6.0
  - @openturn/json@0.6.0
